...
# -*- coding: utf-8 -*-
"""
Generate Supreme Drill-Down GIS Dashboard for Chiang Mai Water Master Plan
- Organic polygon boundaries for both District AND Subdistrict (Zero bounding boxes/rectangles)
- Both left & right maps synchronized on click and hover
- Solves info panel blocking map: Smart Collapsible Bottom Dock (Floating Google Sheet)
- Seamless 3-tier drilldown: อำเภอ > ตำบล > หมู่บ้าน with auto-updating dropdowns and pulsing village beacon
- Direct on-map numbers on both maps
"""
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; margin: 0; padding: 0; font-family: 'Prompt', 'Google Sans', sans-serif; }}
    ...
    /* ========================================================= */
    /* SMART FLOATING BOTTOM DOCK (Does NOT cover the map!)       */
    /* ========================================================= */
    .smart-dock {{
      position: absolute;
      bottom: 16px;
    ...
    .btn-toggle-expand:hover {{ background: #d2e3fc; }}
    .btn-close-dock {{
      background: none;
      border: none;
      color: var(--text-sub);
    ...
    /* Custom On-Map Direct Labels for Districts */
    .onmap-label-risk {{
    ...
    .onmap-label-risk .dname {{ font-weight: 700; color: #d93025; font-size: 11px; }}
    .onmap-label-risk .dstat {{ font-size: 10px; color: #3c4043; font-weight: 600; }}
    ...
    .onmap-label-budget .dname {{ font-weight: 700; color: #1a73e8; font-size: 11px; }}
    .onmap-label-budget .dstat {{ font-size: 10px; color: #3c4043; font-weight: 600; }}
    ...
    /* Pulsing beacon animation for selected village point */
    @keyframes pulseRing {{
...
<body>
  ...
  <!-- Filter & Control Bar (3-Tier Drill-Down: อำเภอ > ตำบล > หมู่บ้าน) -->
  ...
  <script>
    ...
    function onDistrictChange() {{
      ...
      vilSelect.innerHTML = '<option value="all">🏡 ทุกหมู่บ้าน</option>';

      if (selectedDistrict !== 'all') {{
        // Populate subdistricts of this district
        ...
        // Populate villages of this district
    ...
    function onSubdistrictChange() {{
      ...
      if (selectedSubdistrict !== 'all') {{
        const villages = [...new Set(
          VILLAGES_DATA.features
            .filter(f => (selectedDistrict === 'all' || f.properties.district === selectedDistrict) && f.properties.subdistrict === selectedSubdistrict)
            .map(f => f.properties.village)
        )].sort((a,b) => a.localeCompare(b, 'th'));
    ...
    function onVillageChange() {{
      ...
      // Find village feature
      const vFeat = VILLAGES_DATA.features.find(f => {{
        const p = f.properties;
        const matchD = (selectedDistrict === 'all' || p.district === selectedDistrict);
        const matchS = (selectedSubdistrict === 'all' || p.subdistrict === selectedSubdistrict);
        return matchD && matchS && p.village === selectedVillage;
      }});

      if (vFeat) {{
        const [lng, lat] = vFeat.geometry.coordinates;
        isSyncing = true;
        mapRisk.setView([lat, lng], 14);
        mapBudget.setView([lat, lng], 14);
        isSyncing = false;

        // Add pulsing beacon to village
        ...
        showVillageDetails(vFeat.properties);
      }}
    }}
    ...
    function renderAllLayers() {{
      ...
      // Left Map District Layers (Real Polygons)
      ...
      // Right Map District Layers (Real Polygons + Direct Numbers on GIS)
    ...
    function selectDistrictOnBothMaps(name) {{
      ...
      // Load organic subdistrict boundaries inside this district
    ...
    function highlightAndZoomSubdistrict(tamName) {{
      if (tamName === 'all') {{
      ...
      if (subFeat) {{
        ...
        subdistrictGroupRisk = L.geoJSON(subFeat, {{
          style: {{
            color: '#d93025',
            weight: 3.5,
            fillColor: '#d93025',
        ...
        subdistrictGroupBudget = L.geoJSON(subFeat, {{
          style: {{
            color: '#0d47a1',
            weight: 3.5,
            fillColor: '#0d47a1',
        ...
        const bounds = subdistrictGroupRisk.getBounds();
        isSyncing = true;
        mapRisk.fitBounds(bounds, {{ padding: [35, 35], animate: true }});
        mapBudget.fitBounds(bounds, {{ padding: [35, 35], animate: true }});
        isSyncing = false;

        // Show subdistrict details in smart dock
    ...
    function resetToOverview() {{
      ...
      document.getElementById('subdistrictSelect').innerHTML = '<option value="all">🏘️ ทุกตำบล</option>';
      document.getElementById('villageSelect').innerHTML = '<option value="all">🏡 ทุกหมู่บ้าน</option>';
    ...
    function showDistrictDetails(p) {{
      ...
      const budget = (p.total_budget || 0).toLocaleString('th-TH', {{ maximumFractionDigits: 1 }});
    ...
    function showSubdistrictDetails(tamName, distName) {{
      ...
      document.getElementById('dockExpandedBody').innerHTML = `
        ...
        <div style="font-size:0.8rem; color:#5f6368; margin-bottom:8px;">
          รายชื่อหมู่บ้านในตำบล:
        </div>
        <div style="display:flex; flex-wrap:wrap; gap:6px; max-height:160px; overflow-y:auto;">
          ${{vils.map(v => `<span style="background:#e8f0fe; color:#1a73e8; padding:3px 8px; border-radius:10px; font-size:0.76rem; font-weight:600;">${{v.properties.village}} (${{v.properties.priority.split(' ')[0]}})</span>`).join('')}}
        </div>
      `;
      dock.style.display = 'flex';
    }}
    ...
    function showVillageDetails(p) {{
      ...
      `;
      dock.style.display = 'flex';
      // Auto expand on village click to show scores immediately
...
print("Supreme Drill-Down GIS Dashboard generated successfully!")

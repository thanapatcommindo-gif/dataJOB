...
# -*- coding: utf-8 -*-
"""
Generate Supreme Dual Sync GIS Dashboard for Chiang Mai Water Master Plan
- 100% Real Organic Polygon boundaries (Zero bounding boxes / rectangles)
- Direct on-map metric labels (Numbers directly on GIS for both maps)
- Left Map: Risk Numbers (จุดเสี่ยงสูง, หมู่บ้าน) on each district
- Right Map: Budget Numbers (วงเงินล้านบาท, จำนวนโครงการ) on each district
- Synchronized click, hover, zoom, and multi-pillar filtering
"""
  ...
  <script>
    ...
    function selectDistrictOnBothMaps(name) {{
      ...
      for (const [dName, layer] of Object.entries(districtLayersRisk)) {{
        ...
        layer.setStyle({{
          weight: (dName === name) ? 4 : 1.2,
          color: (dName === name) ? '#1a73e8' : '#ffffff',
          fillOpacity: isTarget ? 0.78 : 0.22
        }});
      }}
      ...
      for (const [dName, layer] of Object.entries(districtLayersBudget)) {{
        ...
        layer.setStyle({{
          ...
          fillOpacity: isTarget ? 0.82 : 0.22
        }});
      }}
    ...
    function renderSubdistrictsForDistrict(distName) {{
      ...
      if (subFeats.length > 0) {{
        ...
        subdistrictGroupRisk = L.geoJSON(subFeats, {{
          ...
          onEachFeature: (f, l) => {{
            l.bindTooltip(`ต.${{f.properties.tam_th}}`, {{ direction: 'center', permanent: false }});
          }}
        }}).addTo(mapRisk);
    ...
    function zoomToSubdistrict(tamName) {{
      ...
      if (subFeats.length > 0) {{
        ...
        isSyncing = true;
        mapRisk.fitBounds(bounds, {{ padding: [35, 35] }});
        mapBudget.fitBounds(bounds, {{ padding: [35, 35] }});
        isSyncing = false;
      }}
    }}
...
print("Supreme Dual Sync GIS with on-map direct numbers and pure organic polygons generated successfully!")
